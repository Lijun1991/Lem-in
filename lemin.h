...
# define C1 (!(info->room_name[j] = ft_strdup(info->tmp_room_name[0])))
# define C2 (!(info->room_x[j] = ft_strdup(info->tmp_room_name[1])))
# define C3 (!(info->room_y[j] = ft_strdup(info->tmp_room_name[2])))
...
void			ree_room(t_leminfo *info, char **map);
...
void			print_normal_room(char **map, t_leminfo *info, \
	char *start, char *end);
